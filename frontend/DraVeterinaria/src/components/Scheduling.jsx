...
const Scheduling = () => {
  ...
  useEffect(() => {
    // Si no hay un tipo de servicio seleccionado, no hacemos nada.
    if (!servicio.tipo) {
      setSubtiposServicio([]);
      return;
    }

    // Asegúrate de que servicio.tipo sea el ID numérico esperado por la API
  ...
  const submitServicio = async (e) => {
    ...
    try {
      ...
      // NOTA: Ajusta el body para enviar idSubtipoServicio si tu API lo requiere
      // y si tu endpoint de servicios maneja el idSubtipo como parte de la solicitud.
      await fetch("http://localhost:8090/api/servicios", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          mascota: { idMascota: mascotaData.idMascota },
          // Se envía el id del tipo de servicio (que es el valor de servicio.tipo)
          idTipo: servicio.tipo,
          // Se envía el id del subtipo de servicio (que es el valor de servicio.subtipo)
          idSubtipo: servicio.subtipo, 
  ...
  // -----------------------------
  // Submit Mascota + Tutor (sin cambios)
  // -----------------------------
